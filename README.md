...
- PDF invoice export
- Optional email sending through Cloudflare Pages Functions + Resend
...
When email sending is configured and the user clicks **Send invoice email**, the app sends the recipient, reply-to address, message, and PDF attachment to the Cloudflare Pages Function at `/api/send-invoice`. The function forwards the email through Resend.

## Email sending

Create a free Resend account, verify a sender domain, then add these Cloudflare Pages environment variables:

```text
RESEND_API_KEY=...
FROM_EMAIL=Andras <invoices@yourdomain.com>
```

The email is sent from `FROM_EMAIL`, and replies go to the business email entered during setup.
